...
public class WebFluxEmployeeController {
	...
	public Mono<Resources<Resource<Employee>>> all() {
		...
		return Flux.fromIterable(EMPLOYEES.keySet())
			...
			.collectList()
			.flatMap(resources -> linkTo(methodOn(WebFluxEmployeeController.class).all())
	...
	public Mono<Resource<Employee>> findOne(@PathVariable Integer id) {
		...
		return linkTo(methodOn(WebFluxEmployeeController.class).findOne(id))
			.map(ReactiveLinkBuilder::withSelfRel)
			.zipWith(linkTo(methodOn(WebFluxEmployeeController.class).all()).map(reactiveLinkBuilder -> reactiveLinkBuilder.withRel("employees")))
	...
	public Mono<ResponseEntity<?>> updateEmployee(@RequestBody Mono<Resource<Employee>> employee, @PathVariable Integer id) {
	...
	public Mono<ResponseEntity<?>> partiallyUpdateEmployee(@RequestBody Mono<Resource<Employee>> employee,
