...
public enum IanaLinkRelation implements LinkRelation {
	...
	/**
	 * Ignoring case, is this link relation an IANA standard?
	 *
	 * @param rel
	 * @return boolean
	 */
	public static boolean isIanaRelIgnoreCase(String rel) {
		return rel != null && RELS.stream().anyMatch(s -> s.equalsIgnoreCase(rel));
	}
	...
	public static IanaLinkRelation parse(String rel) {
		...
		return LINK_RELATIONS.stream()
			.filter(linkRelation -> linkRelation.value().equals(rel))
			.findFirst()
			.orElseThrow(() -> new IllegalArgumentException(rel + " is not a valid IANA link relation!"));
	}

	/**
	 * Ignoring case, convert a string-based link relation to a {@link IanaLinkRelation}.
	 *
	 * @param rel as a string, ignoring the case
	 * @return rel as a {@link IanaLinkRelation}
	 */
	public static IanaLinkRelation parseIgnoringCase(String rel) {
