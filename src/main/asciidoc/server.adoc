...
The `WebFluxLinkBuilder` uses Spring’s `UriComponentsBuilder` under the hood to obtain the basic URI information from the current request. Assuming your application runs at http://localhost:8080, this is exactly the URI on top of which you are constructing additional parts. The builder now inspects the given controller method for its request mapping and thus ends up with http://localhost:8080/people. You can also build more nested links as well.
The following example shows how to do so:
...
You can also pass arguments to controller methods. These arguments will be used to create the links.
The following example shows how to do so:
